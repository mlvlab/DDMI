...
def get_activations(files, model, batch_size=50, dims=2048, device='cpu', max_samples=None):
    ...
    for batch in dl:
        ...
        #print(batch.shape)
        #batch = symmetrize_image_data(batch)
        if batch.shape[1] == 1:  # if image is gray scale
            batch = batch.repeat(1, 3, 1, 1)
        #batch = torchvision.transforms.functional.resize(batch, 256, antialias = True)
        #batch = batch.clamp(-1., 1.)
        #batch = unsymmetrize_image_data(batch)
        ...
        if pred.size(2) != 1 or pred.size(3) != 1:
            pred = adaptive_avg_pool2d(pred, output_size=(1, 1)).to(torch.float64)
