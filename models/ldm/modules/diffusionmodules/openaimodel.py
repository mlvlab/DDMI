...
from models.ldm.modules.attention_efficient import SpatialTransformer
from models.d2c_vae.autoencoder_unet import make_attn
...
class UNetModel_Triplane(nn.Module):
    """
    The full UNet model with attention and timestep embedding.
    :param in_channels: channels in the input Tensor.
    :param model_channels: base channel count for the model.
    ...
    def __init__(
        ...
        **ignorekwargs
        ...
        for level, mult in enumerate(channel_mult):
            for _ in range(num_res_blocks):
                ...
                self.input_attns.append(
                            make_attn(ch, attn_type=attn_type)
                        )
        ...
        self.mid_attn = make_attn(ch, attn_type=attn_type)
        self._feature_size += ch
        ...
        for level, mult in list(enumerate(channel_mult))[::-1]:
            for i in range(num_res_blocks + 1):
                ...
                if level and i == num_res_blocks:
                    ...
                    ds //= 2
                self.output_blocks.append(TimestepEmbedSequential(*layers))
                self.output_attns.append(make_attn(ch, attn_type=attn_type))
                self._feature_size += ch
    ...
    def forward(self, x, timesteps=None, cond=None, context=None, y=None,**kwargs):
        ...
        #t_emb = self.time_embedding(timesteps, self.model_channels, repeat_only=False, device = x.device)
        t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=False)
        #t_emb = self.temb_fun(timesteps)
        emb = self.time_embed(t_emb)
