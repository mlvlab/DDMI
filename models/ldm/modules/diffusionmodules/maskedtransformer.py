...
class MDTv2(nn.Module):
    ...
    def forward(self, x, t, y, self_cond=None, enable_mask=True):
        """
        Forward pass of MDT.
        ...
        for block in self.en_inblocks:
            if masked_stage:
                x = torch.utils.checkpoint.checkpoint(self.ckpt_wrapper(block), x, c, None, ids_keep) 
                #x = block(x, c, ids_keep=ids_keep)
            else:
                x = torch.utils.checkpoint.checkpoint(self.ckpt_wrapper(block), x, c, None, None) 
                #x = block(x, c, ids_keep=None)
            skips.append(x)

        for block in self.en_outblocks:
            if masked_stage:
                x = torch.utils.checkpoint.checkpoint(self.ckpt_wrapper(block), x, c, skips.pop(), ids_keep) 
                #x = block(x, c, skip=skips.pop(), ids_keep=ids_keep)
            else:
                x = torch.utils.checkpoint.checkpoint(self.ckpt_wrapper(block), x, c, skips.pop(), None) 
        ...
        for i in range(len(self.de_blocks)):
            ...
            x = torch.utils.checkpoint.checkpoint(self.ckpt_wrapper(block), x, c, this_skip, None) 
