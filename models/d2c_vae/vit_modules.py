...
#from models.d2c_vae.autoencoder_unet import MemoryEfficientAttnBlock1DHeads
...
class EfficientTimeSformerEncoder(nn.Module):
    def __init__(
        self,
        *,
        dim = 512,
        ...
        for _ in range(depth):
            ff = FeedForward(dim, dropout = ff_dropout)
            #time_attn = Attention(dim, dim_head = dim_head, heads = heads, dropout = attn_dropout)
            time_attn = MemoryEfficientAttnBlock1DHeads(dim, dim_heads = dim_head, num_heads = heads)
            #spatial_attn = Attention(dim, dim_head = dim_head, heads = heads, dropout = attn_dropout)
            spatial_attn = MemoryEfficientAttnBlock1DHeads(dim, dim_heads = dim_head, num_heads = heads)
