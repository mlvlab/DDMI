...
from convocc.src.layers import ResnetBlockFC
from torch_scatter import scatter_mean, scatter_max
from convocc.src.common import coordinate2index, normalize_coordinate, normalize_3d_coordinate, map2local
from convocc.src.encoder.unet import UNet
from convocc.src.encoder.unet3d import UNet3D
...
class LocalPoolPointnet(nn.Module):
    ...
    def __init__(self, c_dim=128, dim=3, hidden_dim=128, scatter_type='max', 
                 unet=None, plane_resolution=None, grid_resolution=None, plane_type='xz', padding=0.1, n_blocks=5):
        super().__init__()
        self.c_dim = c_dim
    ...
    ### Slight modification for D2C-VAE
    def forward(self, p, is_latent = False):
        ...
        if not is_latent:
            ...
            if 'xz' in self.plane_type:
                fea['xz'] = self.unet(latent = p[1])
                #print('fea xz shape', fea['xz'].shape)
            if 'xy' in self.plane_type:
                fea['xy'] = self.unet(latent = p[0])
            if 'yz' in self.plane_type:
                fea['yz'] = self.unet(latent = p[2])
        return fea
