...
from convocc.src import conv_onet
...
# Models
def get_model(cfg, device=None, dataset=None, unet = None, decoder = None):
    ...
    model = method_dict[method].config.get_model(
        cfg, device=device, dataset=dataset, unet = unet, decoder = decoder)
    return model
...
def get_generator(cfg):
    ...
    '''
    method = cfg['method']
    generator = method_dict[method].config.get_generator(cfg)
    return generator
...
def get_dataset(mode, cfg, return_idx=False):
    ...
    if dataset_type == 'Shapes3D':
        ...
        dataset = data.Shapes3dDataset(
            ...
            cfg = cfg,
            t2s = cfg['data']['text2shape'],
            t2s_path = cfg['data']['text2shape_path']
        )
    else:
