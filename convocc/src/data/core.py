...
class Shapes3dDataset(data.Dataset):
    ...
    def __init__(self, dataset_folder, fields, split=None,
                 categories=None, no_except=True, transform=None, cfg=None, t2s=False, t2s_path=None):
        ...
        self.cfg = cfg
        self.t2s = t2s # Text2shape

        if t2s:
            assert t2s_path is not None
            self.text_prompt = pd.read_csv(t2s_path)
    ...
    def __getitem__(self, idx):
        ...
        if self.t2s:    
            data['id'] = model
            descript_list = []
            idx = self.text_prompt.index[self.text_prompt['modelId'] == model]
            des = list(self.text_prompt['description'][idx])
            length = len(des)
            if length == 0:
                data['des'] = ""
            else:
                randint = random.randint(0, length-1)
                data['des'] = des[randint][:77]
